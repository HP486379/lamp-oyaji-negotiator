...
const SLOT_REGISTRY = deepFreeze({
  ...
  slots: [...UNIVERSAL_SLOT_TEMPLATES, "pattern_primary_capability"].map((slotTemplateId) => ({ slotTemplateId, satisfactionRuleId: "raw_answer_for_issued_question", registryVersion: REGISTRY_VERSION_SET.slotTemplate })),
});
...
const newState = () => ({
  registries: REGISTRIES, rawEvents: new Map(), revisions: new Map(), evidence: new Map(),
...
const deriveSourcesForAnswer = (state, question, revision, evidenceId) => {
  ...
  addClaim(state, { id: capId, kind: "capability", targetId: question.targetId, evidenceId, scopeKey: question.scopeKey });
  addClaim(state, { id: decisionId, kind: "decision", targetId: question.targetId, evidenceId, scopeKey: question.scopeKey });
};
const initializeQuestions = (state) => {
  for (const definition of state.registries.slotRegistry.slots) {
    ...
    state.questions.set(questionId, deepFreeze({ id: questionId, targetId, slotTemplateId: definition.slotTemplateId, scopeKey: "global", answeredByEventId: null }));
...
export function submitRawInitialInput({ authority, event }) {
  ...
  if (state.coreUserValue) throw new Error("initial input is already recorded");
  ...
  initializeQuestions(state); return { accepted: true, replay: false };
}
export function listPendingQuestions({ authority }) {
  const state = internal(authority);
  return [...state.questions.values()].filter((question) => !question.answeredByEventId).map((question) => ({ questionId: question.id, slotTemplateId: question.slotTemplateId }));
}
export function submitRawUserAnswer({ authority, event }) {
  ...
  const rule = state.registries.slotRegistry.slots.find((item) => item.slotTemplateId === question.slotTemplateId);
  if (!rule || rule.satisfactionRuleId !== "raw_answer_for_issued_question" || !currentEvidence(state, evidenceId, question.targetId)) throw new Error("slot satisfaction rule did not pass");
  state.slots.set(question.slotTemplateId, { slotTemplateId: question.slotTemplateId, status: "satisfied", evidenceId, currentness: "current" });
  state.questions.set(question.id, { ...question, answeredByEventId: raw.eventId }); deriveSourcesForAnswer(state, question, revision, evidenceId);
